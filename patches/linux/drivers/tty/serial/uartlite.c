...
/* chiron: drive RX by polling since the platform has no wired uartlite IRQ. */
#define CHIRON_RX_POLL 1
...
struct uartlite_data {
	...
	unsigned int baud;
	tcflag_t cflags;
	struct uart_port *port;		/* back-pointer for the RX poll timer */
	struct timer_list rx_poll;	/* RX fallback when no IRQ is wired */
};
...
/*
 * RX polling fallback. The uartlite RX path is normally driven by ulite_isr,
 * but the chiron golden model / RTL have no wired uartlite interrupt (no PLIC
 * delivery), so the ISR never fires and typed characters would never be read
 * out of the RX FIFO. Mirror the TX drain hack in ulite_start_tx with a periodic
 * timer that pulls any pending RX bytes into the tty layer. Harmless on real HW,
 * where the IRQ also handles RX and the FIFO is simply empty when polled.
 */
#ifdef CHIRON_RX_POLL
static void ulite_rx_poll(struct timer_list *t)
{
	...
	spin_lock_irqsave(&port->lock, flags);
	while (ulite_receive(port, uart_in32(ULITE_STATUS, port)))
		busy = 1;
	spin_unlock_irqrestore(&port->lock, flags);
...
static int ulite_startup(struct uart_port *port)
{
	...
	/* Start the RX poll timer (see ulite_rx_poll) for IRQ-less platforms. */
	pdata->port = port;
#ifdef CHIRON_RX_POLL
	timer_setup(&pdata->rx_poll, ulite_rx_poll, 0);
	mod_timer(&pdata->rx_poll, jiffies + 1);
#endif
