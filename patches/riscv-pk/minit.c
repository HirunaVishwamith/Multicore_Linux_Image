...
// Unused (and unassembleable on toolchains predating the menvcfg CSR name)
// when BBL_BOOT_MACHINE is set: bbl.c's #ifdef BBL_BOOT_MACHINE branch calls
// enter_machine_mode() instead and never references this function -- chiron
// always builds with --enable-boot-machine, so guard it out like the other
// S-mode-only setup (delegate_traps()) above.
#ifndef BBL_BOOT_MACHINE
